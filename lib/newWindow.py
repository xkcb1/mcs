...
class MyWindow(QWidget, Ui_Form):
    ...
    def _init_main_window(self):
        ...
        self.label_2.setObjectName('title_')
    ...
    def resizeEvent(self, QResizeEvent):
        # 自定义窗口调整大小事件
        # 改变窗口大小的三个坐标范围
        self._right_rect = [QPoint(x, y) for x in range(self.width() - 20, self.width() - 14)
                            for y in range(self.widget.height() + 20, self.height() - 5)]
        self._bottom_rect = [QPoint(x, y) for x in range(-6, self.width() - 12)
                             for y in range(self.height() - 20, self.height() - 14)]
        self._corner_rect = [QPoint(x, y) for x in range(self.width() - 20, self.width() - 14)
                             for y in range(self.height() - 20, self.height()-14)]
    ...
    def mousePressEvent(self, event):
        ...
        if (event.button() == Qt.LeftButton) and (event.pos() in self._corner_rect):
            ...
            event.accept()
        elif (event.button() == Qt.LeftButton) and (15 < event.y() < self.widget.height()+15):
            # 鼠标左键点击标题栏区域
            self._move_drag = True
            self.move_DragPosition = event.globalPos() - self.pos()
            event.accept()
        ChoicePanel.CheckRemoveThePanel(event.pos())
