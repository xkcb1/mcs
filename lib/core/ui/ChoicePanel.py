...
def OutTheChoicePanel(sender:QPushButton,self,WindowPos) -> None:
    ...
    if Pos_Y - window_y + ChoicePanel_Height < window_height:
        #to bottom
        y = Pos_Y - window_y + widget_Height - Frame_Y
        b = 'bottom'
    else:
        y = Pos_Y - window_y - ChoicePanel_Height - Frame_Y
        b = 'top'
        #to top
    #通过展现方向来控制sender的圆角
    sender.setObjectName('WindowName')
    if b == 'bottom':
        sender.setStyleSheet("#WindowName{border: 1px solid #ccc;border-top-left-radius: 7px;border-top-right-radius: 7px;background-color: #cbe8ff;background-image: url(./img/bottom_blue.png);background-position:right center;background-repeat:no-repeat;}")
    else:
        sender.setStyleSheet("#WindowName{border: 1px solid #ccc;border-bottom-left-radius: 7px;border-bottom-right-radius: 7px;background-color: #cbe8ff;background-image: url(./img/bottom_blue.png);background-position:right center;background-repeat:no-repeat;}")
    #默认是 right-bottom
    for i in ChoicePanelDict:
        self.findChild(QWidget,i).deleteLater()
        ChoicePanelDict = {}
    ...
    ChoicePanelDict["ChoicePanel_"+str(ChoicePanelCount)] = [x,y,ChoicePanel_Width,ChoicePanel_Height,a,b]
    DrawChoicePanel(ChoicePanel,a,b,"ChoicePanel_"+str(ChoicePanelCount),sender.text(),sender)
    ChoicePanel.show()
    #实现ChoicePanel样式和细节
    ChoicePanelCount += 1
    pass
        ...
        #开始判断是否删除 弹出框
            ...
            if ClickPos.x() < pos[0] or ClickPos.x() > (pos[0]+pos[2]):
                ...
                if ClickPos.y() < pos[1] or ClickPos.y() > (pos[1]+pos[3]):
                    ...
                    if pos[5] == 'bottom':
                        LastSender.setStyleSheet("border: 1px solid #ccc;border-top-left-radius: 7px;border-top-right-radius: 7px;background-color: #cbe8ff;background-image: url(./img/bottom_blue.png);background-position:right center;background-repeat:no-repeat;")
                    else:
                        LastSender.setStyleSheet("border: 1px solid #ccc;border-bottom-left-radius: 7px;border-bottom-right-radius: 7px;background-color: #cbe8ff;background-image: url(./img/bottom_blue.png);background-position:right center;background-repeat:no-repeat;")
                    This_Self.effect_shadow = QGraphicsDropShadowEffect()
                    This_Self.effect_shadow.setOffset(0,0) # 偏移
                    This_Self.effect_shadow.setBlurRadius(30) # 阴影半径
                    This_Self.effect_shadow.setColor(QtCore.Qt.gray) # 阴影颜色
                    for i in ChoicePanelDict:
                        This_Self.findChild(QWidget,i).setGraphicsEffect(This_Self.effect_shadow) # 将设置套用到button窗口中
def DrawChoicePanel(Widget:QWidget,marginA,marginB,Name,ThisPanel,sender:QPushButton) -> None:
    ...
    SmallTitle_1.setMaximumHeight(30)
    SmallTitle_1.setMinimumHeight(30)
    ...
    for Name in WindowMenuThis.WindowMenuInChoice['Standard']:
        ...
        This_Self.ChoiceButton.clicked.connect(functools.partial(ChangePanel, Name))
        This_Self.ChoiceButton.setIcon(QIcon(WindowMenuThis.WindowMenuInChoice['Standard'][Name]['icon']))
        if Name == ThisPanel:
    ...
    SmallTitle_2.setMaximumHeight(30)
    SmallTitle_2.setMinimumHeight(30)
    ...
    for Name in WindowMenuThis.WindowMenuInChoice['Convention']:
        ...
        This_Self.ChoiceButton.setMinimumHeight(24)
        This_Self.ChoiceButton.clicked.connect(functools.partial(ChangePanel, Name))
        This_Self.ChoiceButton.setIcon(QIcon(WindowMenuThis.WindowMenuInChoice['Convention'][Name]['icon']))
        if Name == ThisPanel:
            This_Self.ChoiceButton.setObjectName('ChoiceButton_this')
            #This_Self.ChoiceButton.clicked.connect(None)
    ...
    SmallTitle_3.setMaximumHeight(30)
    SmallTitle_3.setMinimumHeight(30)
    ...
    for Name in WindowMenuThis.WindowMenuInChoice['extend']:
        ...
        This_Self.ChoiceButton.clicked.connect(functools.partial(ChangePanel, Name))
        #新方法是用childAT办法
        This_Self.ChoiceButton.setIcon(QIcon(WindowMenuThis.WindowMenuInChoice['extend'][Name]['icon']))
        if Name == ThisPanel:
            This_Self.ChoiceButton.setObjectName('ChoiceButton_this')
            #This_Self.ChoiceButton.clicked.connect(None)
    ...
    SmallTitle_4.setMaximumHeight(30)
    SmallTitle_4.setMinimumHeight(30)
    ...
    for Name in WindowMenuThis.WindowMenuInChoice['tools']:
        ...
        This_Self.ChoiceButton.clicked.connect(functools.partial(ChangePanel, Name))
        #新方法是用childAT办法
        This_Self.ChoiceButton.setIcon(QIcon(WindowMenuThis.WindowMenuInChoice['tools'][Name]['icon']))
        if Name == ThisPanel:
...
def ChangePanel(Name):
    ...
    #准备开始初始化新的panel
    #应该在这里就去获取到点击的按钮
    Starter.PanelStarter(This_Sender,This_Self,Name)#把sender的text()->str作为参数传入，初始化函数
    pass