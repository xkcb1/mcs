...
public class ForceLoadCommand {
    ...
    private static int executeChange(ServerCommandSource source, ColumnPos from, ColumnPos to, boolean forceLoaded) throws CommandSyntaxException {
        ...
        if (r == 1) {
            source.sendFeedback(() -> Text.translatable("commands.forceload." + (forceLoaded ? "added" : "removed") + ".single", chunkPos2, registryKey.getValue()), true);
        } else {
            ChunkPos chunkPos3 = new ChunkPos(m, n);
            ChunkPos chunkPos4 = new ChunkPos(o, p);
            source.sendFeedback(() -> Text.translatable("commands.forceload." + (forceLoaded ? "added" : "removed") + ".multiple", chunkPos2, registryKey.getValue(), chunkPos3, chunkPos4), true);
        }
        return r;
    }
}
