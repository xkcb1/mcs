...
public class DataCommand {
    ...
    private static final Dynamic2CommandExceptionType MODIFY_INVALID_SUBSTRING_EXCEPTION = new Dynamic2CommandExceptionType((startIndex, endIndex) -> Text.translatable("commands.data.modify.invalid_substring", startIndex, endIndex));
        ...
        for (ObjectType objectType : TARGET_OBJECT_TYPES) {
            objectType.addArgumentsToBuilder(literalArgumentBuilder, builder -> {
                ...
                for (ObjectType objectType2 : SOURCE_OBJECT_TYPES) {
                    ...
                    subArgumentAdder.accept(argumentBuilder, operation -> objectType2.addArgumentsToBuilder(CommandManager.literal("string"), builder -> ((ArgumentBuilder)builder.executes(context -> DataCommand.executeModify(context, objectType, operation, DataCommand.mapValues(DataCommand.getValues(context, objectType2), value -> value)))).then(((RequiredArgumentBuilder)CommandManager.argument("sourcePath", NbtPathArgumentType.nbtPath()).executes(context -> DataCommand.executeModify(context, objectType, operation, DataCommand.mapValues(DataCommand.getValuesByPath(context, objectType2), value -> value)))).then(((RequiredArgumentBuilder)CommandManager.argument("start", IntegerArgumentType.integer()).executes(context -> DataCommand.executeModify(context, objectType, operation, DataCommand.mapValues(DataCommand.getValuesByPath(context, objectType2), value -> DataCommand.substring(value, IntegerArgumentType.getInteger(context, "start")))))).then(CommandManager.argument("end", IntegerArgumentType.integer()).executes(context -> DataCommand.executeModify(context, objectType, operation, DataCommand.mapValues(DataCommand.getValuesByPath(context, objectType2), value -> DataCommand.substring(value, IntegerArgumentType.getInteger(context, "start"), IntegerArgumentType.getInteger(context, "end"))))))))));
                }
